...
def triangle (p1, p2, p3, color): # Рисуем треугольник по трем точкам - p1, p2, p3
	...
	# Закрашивать треугольник
        ...
        for i in range(x1, x2+1):
                lineP([i, y1 + ((i - x1)*(y2 - y1))//(x2 - x1)], [i, y1 + ((i - x1)*(y3 - y1))//(x3 - x1)], color)

        for i in range(x2, x3+1):
                lineP([i, y2 + ((i - x2)*(y3 - y2))//(x3 - x2)], [i, y1 + ((i - x1)*(y3 - y1))//(x3 - x1)], color)
